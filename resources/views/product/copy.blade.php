...
<div class="row">
	<div class="col-md-12">
		<form class="form-horizontal form-row-seperated"   method="post" action="{{ url('/')}}/store-product" enctype="multipart/form-data">
		 {!! csrf_field() !!}
		    <input type="hidden" name="copy_row_id" value="{{  $data['single_record']->product_row_id }}" />
			<div class="portlet">
						...
						<div class="tab-content">						
							<div class="tab-pane active" id="tab_general">								
								<div class="form-body">
									<div class="form-group">
										...
										<div class="col-md-4">
											<input type="text" class="form-control" id="product_name" name="product_name" value="{{  $data['single_record']->product_name }}"  placeholder="" required> 
										</div>
										<div class="col-md-4">
									...
									<div class="form-group">
										...
										<div class="col-md-10">
											<div style="float:left;"><input type="text" name="aku_code" maxlength="5"   id="aku_code" class="form-control input-medium"  placeholder="5 digit # only"></div><div style="float:left;padding:0 0 0 10px">  <button type="button" id="aku_code_btn" class="btn green btn-large">Generate Art Keeping Unit (AKU)</button></div>											
									...
									<div class="form-group">
										...
										<div class="col-md-10">		
											...
											<textarea class="wysihtml5 form-control" rows="8" id="description_new" name="product_long_description">{{ $data['single_record']->product_long_description }}</textarea>											
										</div>
									</div>
											...
											<div action="{{ url('/')}}/upload-product-image" class="dropzone dropzone-file-area sortable" id="productImagesDropzone" style="margin:0; width: 400px; margin-top: 0px;background-color:#f1f1f1">
												...
												@if($data['single_record']->product_image)
													<div id="uploaded_product_image_box" class="dz-preview dz-processing dz-image-preview dz-success dz-complete design-image-preview "> 
														<div class="dz-image">
															<img data-dz-thumbnail="" alt="product image" src="{{ url('/')}}/public/uploads/product_images/{{ $data['user_id']}}/{{ $data['single_record']->product_image }}"  /> 
														</div>  
														...
														<a href="javascript:;" '=""   class="btn red btn-sm btn-block product-image-delete product-image-delete-copy">Remove</a>
													</div>
													<input type="hidden" name="hidden_primary_image" id="hidden_primary_image" value="{{ $data['single_record']->product_image }}">
												@endif	
											</div>  
										</div>
									</div>
							...
							<div class="tab-pane" id="tab_details">
								...
								<div class="form-group">
									...
									<div class="col-md-10">
										<input type="text" class="form-control" value="{{  $data['single_record']->update_source }}" name="update_source" placeholder=""> </div>
								</div>
							...
							<div class="tab-pane" id="tab_price_level">							 
								<div class="form-body">
									@foreach($data['price_list'] as $row)
										...
										<div class="col-md-2"> 
											<input type="text" class="form-control"  value="{{ isset($data['price_level_amount'] [$row->customer_group_row_id]) ? $data['price_level_amount'] [$row->customer_group_row_id] : '' }}" id="product_level_amount_{{$row->customer_group_row_id}}" name="product_level_amount[{{$row->customer_group_row_id}}][]" placeholder="0.00" >  
										</div>
...
Dropzone.options.HireImageDropzone = {
  init: function() {
	...
	$(".product-image-delete-copy").click(function(){
	  //var uploaded_design_image_name = $(this) . attr('image_name');
	  $('#hidden_primary_image').val('');
	  $('.uploaded_product_image_box').hide('');
