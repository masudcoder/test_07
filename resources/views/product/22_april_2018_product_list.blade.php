...
<div class="row">
		...
		<td>
			@if($row->product_image)
				@if($row->image_external_link)
					...
					Thumb Processing
				@else
				<img src="{{ asset('/')}}public/uploads/product_images/{{ $data['user_id'] }}/{{$row->product_image}}" alt="product Image" style="width:100px;height:80px" />
				@endif
			@else
	...
	@endforeach
	<tr><td colspan="7" style="text-align: center"> {{ $data['products_list']->links() }} </td></tr>
	</tbody>
	</table>
